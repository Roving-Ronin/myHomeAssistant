...
// instance's own `total:` sensor's object_id_hash (XORed with the version
// prefix's hash), so each instance gets its own storage automatically - no
// YAML-side configuration needed.
//
...
void WaterStatistics::setup() {
  ...
  // Per-instance key: the version-prefix hash XORed with this instance's
  // own object_id_hash (a uint32_t ESPHome already derives per-entity from
  // the `total:` sensor's object_id - unique across instances without
  // needing a string-based object_id getter, which isn't exposed on Sensor
  // in this ESPHome version).
  uint32_t pref_key_hash = fnv1_hash(PREF_PREFIX) ^ this->total_->get_object_id_hash();
  this->pref_ = global_preferences->make_preference<water_data_t>(pref_key_hash);
  bool loaded = this->pref_.load(&this->water_);
  if (loaded) {
    ESP_LOGI(TAG, "Loaded Water NVS (key=%u): today=%f, yesterday=%f, week=%f, month=%f, year=%f, quarter=%f",
             pref_key_hash, this->water_.start_today, this->water_.start_yesterday, this->water_.start_week,
    ...
    ESP_LOGW(TAG, "No Water NVS data loaded (key=%u), starting fresh", pref_key_hash);
