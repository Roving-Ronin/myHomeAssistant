...
- **Adaptive history rows** — Past tab uses actual recorded timestamps rather than fixed slots, so it works correctly even when history is sparse
...
> **Note:** The Past Events tab uses separate price sensors (`past_buy_price` / `past_sell_price`) because the MPC price sensors only record sparse state history (updates only when value changes). Live spot market sensors like Amber update frequently and provide dense history suitable for past cost calculations.
...
# Power forecast sensors (Future Decisions tab)
...
# Actual price sensors for Past Events tab cost calculations
# Use live spot market sensors that record dense history
# Override these with your own tariff sensors if not using Amber
...
EMHASS MPC sensors only write a new HA state when their value **changes** between optimization runs. This means:

- **Power sensors** (`mpc_batt_power`, `mpc_load_power` etc.) — record a new state every time EMHASS publishes a different value, typically every few runs
- **Price sensors** (`mpc_general_price`, `mpc_feed_in_price`) — only change when the spot price moves, so history can be very sparse
- **SoC sensor** (`mpc_batt_soc`) — only changes when the battery SoC changes enough between runs

This is why the Past tab uses separate `past_buy_price` / `past_sell_price` sensors pointing to your live tariff feed (e.g. Amber), which record state on every poll regardless of value change.
...
| Version | Changes |
|---|---|
| v2.0.9 | Fix Past tab rows all being skipped — `_pwrMult` W→kW conversion made values fall below 10W skip threshold |
| v2.0.8 | Past tab uses actual recorded timestamps instead of fixed 5-min slots — correctly handles sparse history |
| v2.0.7 | Fix Past tab blank — skip condition gated on SoC=0 which had no history, filtering every row |
| v2.0.6 | Separate `past_buy_price` / `past_sell_price` sensors for Past tab — uses dense Amber history instead of sparse MPC price sensors |
| v2.0.5 | Fix daily Cost/Profit sum sign (profit shown as cost); add 10W noise floor clamp to suppress sensor noise |
